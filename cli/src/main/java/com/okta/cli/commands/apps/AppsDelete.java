...
public class AppsDelete implements Callable<Integer> {
    ...
    private boolean deleteApp(Application app) {
        ...
        if (!force && !standardOptions.getEnvironment().isInteractive()) {
            out.writeLine("Application '" + app.getId() + "' has not been deactivated, use '--force' to delete it");
            return false;
        }

        // prompt if needed
        if (force || standardOptions.getEnvironment().prompter()
